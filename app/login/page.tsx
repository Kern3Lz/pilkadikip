...
import { ShieldCheck, UserCheck, Lock, AlertCircle, ArrowRight, KeyRound } from "lucide-react";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (isAdminTab) {
      ...
      if (!identifier || !password) {
        setErrorMessage("Email / NIM dan kata sandi wajib diisi.");
        return;
      }
    }
    ...
    try {
      ...
      if (isAdminTab) {
        ...
        if (data.voter?.has_voted) {
          router.push("/success?already=true");
        } else {
          router.push("/vote");
        }
      }
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-batik-subtle">
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-md bg-white rounded-2xl border-2 border-brand-border shadow-md overflow-hidden">
          {/* Card Header */}
          ...
          <form onSubmit={handleLogin} className="p-6 space-y-4">
            ...
            {!isAdminTab ? (
              <>
                <div>
                  ...
                  <span className="text-[11px] text-gray-500 mt-1 block">
                    Gunakan akun yang telah didaftarkan dalam Daftar Pemilih Tetap (DPT).
                  </span>
                </div>
              ...
              <div>
                <div className="p-3 bg-amber-50 rounded-xl border border-brand-gold/30 text-xs text-amber-900 mb-3 flex items-center gap-2">
                  ...
                  <span>Autentikasi Panitia &amp; Rekapitulasi Suara Pilkadikip</span>
                </div>
            ...
            >
              {loading ? (
                ...
                <>
                  <span>{isAdminTab ? "Masuk Portal Admin" : "Masuk ke Bilik Suara"}</span>
